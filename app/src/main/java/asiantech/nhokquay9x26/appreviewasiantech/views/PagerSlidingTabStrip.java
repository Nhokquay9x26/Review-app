...
package asiantech.nhokquay9x26.appreviewasiantech.views;
...
//                        ((IconTabProvider) pager.getAdapter()).getPageIconResId(i));
    ...
    private void addTabView(int position){
        ...
        View tab = ((IconTabProvider)pager.getAdapter()).getView(position);
        view.addView(tab);
        addTab(position, view);
    }
    private void addCustomTab(final int position, String title, int resId) {
        LinearLayout view = new LinearLayout(getContext());
        view.setOrientation(LinearLayout.VERTICAL);
        view.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
        view.setGravity(Gravity.CENTER);
        ...
        txtTitle.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT));
        txtTitle.setTextSize(TypedValue.COMPLEX_UNIT_SP, 12);
        int padding = convertDPToPixels(getContext(), 4);
        txtTitle.setPadding(0,0,0, convertDPToPixels(getContext(), 4));
        imgIcon.setPadding(padding, padding, padding, padding);

        view.addView(imgIcon);
        view.addView(txtTitle);

        addTab(position, view);

        // Set size for image view
        LinearLayout.LayoutParams params = (LinearLayout.LayoutParams)imgIcon.getLayoutParams();
        params.width = convertDPToPixels(getContext(), 30);
        params.height = convertDPToPixels(getContext(), 30);
    }
    ...
    private void addTab(final int position, View tab) {
        ...
        tab.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (pager.getAdapter() instanceof IconTabProvider) {
                    ((IconTabProvider)pager.getAdapter()).onTabClick(position, pager.getCurrentItem() == position);
                }
    ...
    private class PageListener implements OnPageChangeListener {
        ...
        public void onPageSelected(int position) {
            ...
            for (int i = 0; i < tabsContainer.getChildCount(); i++) {
                layoutTab = tabsContainer.getChildAt(i);
                if (layoutTab instanceof ViewGroup) {
                    for (int j = 0; j < ((ViewGroup) layoutTab).getChildCount(); j++) {
    ...
    public void setTextColor(ColorStateList stateList) {
        this.tabTextColorStateList = stateList;
        updateTabStyles();
    }
